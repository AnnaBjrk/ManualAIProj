...
class Manuals(Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id"))
    file_name: Mapped[str] = mapped_column(String(2048), nullable=False)
...
class UserFileDisplays(Base):
    ...
    # This should be changed in UserFileDisplays model
    file_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("manuals.id"))
