...
from app.api.v1.core.models import Manuals, Users, UserFileDisplays
...
def store_document_s3(
    ...
    # Use the passed client or create one if not provided
...
def get_manual_url_for_download(
    ...
    try:
        ...
        file_stmt = select(Manuals).where(Manuals.id == file_id)
        ...
        # Then check if the user has access through either:
        # 1. They uploaded the file, OR
        # 2. They have added it to their UserFileDisplays
        user_is_owner = file_upload.user_id == current_user.id

        # Check if user has this file in their UserFileDisplays
        display_stmt = select(UserFileDisplays).where(
            UserFileDisplays.file_id == file_id,
            UserFileDisplays.user_id == current_user.id,
            UserFileDisplays.remove_from_view == False
        )
        display_result = db.execute(display_stmt)
        user_display = display_result.scalar_one_or_none()

        if not (user_is_owner or user_display):
            raise HTTPException(
                status_code=403, detail="You don't have access to this file")
